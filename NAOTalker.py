...
class NAOTalker(Talker):
    ...
    def say(self, to_say: str, signal_done: bool = True):
        """
        Speaks the string to_say

        Args:
            to_say (str): The string to speak
            signal_done (bool): If NAO should signal when speaking is done via winking
        """
        for token in ["'",'"']:
    ...
    def stream_say(self, to_say: "Generator<str,None,None>"):
        ...
        for mes in to_say:
            ...
            while res.count('\n') >= 1:
                pos = res.find('\n')
                print(res[0:pos])
                self.say(res[0:pos],signal_done=False)
                res = res[pos+1:]
        print(res)
        self.say(res,signal_done=True)
